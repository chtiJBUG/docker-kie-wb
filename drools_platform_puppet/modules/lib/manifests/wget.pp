...
define lib::wget ($src, $destination, $timeout = 3000, $user = 'root') {
  ...
  exec { "${name}wget":
    ...
    command => "/usr/bin/wget --timeout=$timeout $src -O $destination/${name}", # $src$name is the full link of the file to download
